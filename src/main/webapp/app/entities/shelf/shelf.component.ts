...
export class ShelfComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.shelfService
      ...
      .subscribe(
        (res: HttpResponse<IShelf[]>) => this.paginateShelves(res.body, res.headers),
  ...
  trackId(index: number, item: IShelf) {
    return item.id;
  }
  ...
  protected paginateShelves(data: IShelf[], headers: HttpHeaders) {

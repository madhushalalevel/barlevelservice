...
export class InventoryStockComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.inventoryStockService
      ...
      .subscribe(
        (res: HttpResponse<IInventoryStock[]>) => this.paginateInventoryStocks(res.body, res.headers),
  ...
  trackId(index: number, item: IInventoryStock) {
    return item.id;
  }

  registerChangeInInventoryStocks() {
    this.eventSubscriber = this.eventManager.subscribe('inventoryStockListModification', response => this.loadAll());
  }
  ...
  protected paginateInventoryStocks(data: IInventoryStock[], headers: HttpHeaders) {

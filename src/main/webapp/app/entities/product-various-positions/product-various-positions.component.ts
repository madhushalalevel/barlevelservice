...
export class ProductVariousPositionsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.productVariousPositionsService
      ...
      .subscribe(
        (res: HttpResponse<IProductVariousPositions[]>) => this.paginateProductVariousPositions(res.body, res.headers),
  ...
  trackId(index: number, item: IProductVariousPositions) {
    return item.id;
  }

  registerChangeInProductVariousPositions() {
    this.eventSubscriber = this.eventManager.subscribe('productVariousPositionsListModification', response => this.loadAll());
  }
  ...
  protected paginateProductVariousPositions(data: IProductVariousPositions[], headers: HttpHeaders) {

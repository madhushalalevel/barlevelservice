...
/**
 * REST controller for managing {@link com.bar.level.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
     ...
     */
    @GetMapping("/products")
    public ResponseEntity<List<ProductDTO>> getAllProducts(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("productimage-is-null".equals(filter)) {
            log.debug("REST request to get all Products where productImage is null");
            return new ResponseEntity<>(productService.findAllWhereProductImageIsNull(),
                    HttpStatus.OK);
        }
        if ("inventory-is-null".equals(filter)) {
            log.debug("REST request to get all Products where inventory is null");
            return new ResponseEntity<>(productService.findAllWhereInventoryIsNull(),
                    HttpStatus.OK);
        }

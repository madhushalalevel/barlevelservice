...
import com.bar.level.service.InventoryService;
...
/**
 * REST controller for managing {@link com.bar.level.domain.Inventory}.
 */
@RestController
@RequestMapping("/api")
public class InventoryResource {
    ...
    public ResponseEntity<List<InventoryDTO>> getAllInventories(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("inventorystock-is-null".equals(filter)) {
            log.debug("REST request to get all Inventorys where inventoryStock is null");
            return new ResponseEntity<>(inventoryService.findAllWhereInventoryStockIsNull(),
                    HttpStatus.OK);
        }
